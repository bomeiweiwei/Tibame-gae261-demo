...
def Ask(user_input: str):
    ...
    azure_ai_client = create_ai_langchain(AiType.AZURE)
    azure_response = azure_ai_client.chat(system_prompt, user_prompt)
...
def AskWithImage(user_input: str, image_url: str):
    ...
    azure_ai_client = create_ai_langchain(AiType.AZURE)
    azure_response = azure_ai_client.chat_with_images(system_prompt, human_messages)
