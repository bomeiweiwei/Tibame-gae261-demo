...
def Ask(user_input: str):
    ...
    gemini_ai_client = create_ai_langchain(AiType.GEMINI)
    gemini_response = gemini_ai_client.chat(system_prompt, user_prompt)

    return gemini_response[0]["text"]

def AskWithImage(user_input: str, image_url: str):
    system_prompt, human_messages = build_image_prompt(user_input, [image_url])

    gemini_ai_client = create_ai_langchain(AiType.GEMINI)
    gemini_response = gemini_ai_client.chat_with_images(system_prompt, human_messages)
